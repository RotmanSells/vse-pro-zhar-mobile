...
- `src/infrastructure/product-api-client.ts` is the public HTTP trust boundary for
  `GET /products` and `GET /products/:id`; `src/application/catalog/product.ts`,
...
- `src/presentation/catalog/catalog-shell.tsx` coordinates the existing Category and Product
  Application ports: it selects the first loaded Category, keeps the selected Category in
  Presentation state and passes only matching `categoryId` Products to the view shells.
...
The catalog displays Backend-confirmed Categories and visible Products. Product cards show
name, formatted `basePriceMinor` and a details action. Details show only the approved
