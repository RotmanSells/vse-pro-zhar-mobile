...
secret and has no production default. A physical phone must use the computer's LAN address,
for example `http://192.168.1.105:3100`; `127.0.0.1` from the phone points back to the phone.
...
production must use Phone + SMS OTP after security, integration and owner-approved client
acceptance.
...
## Manual acceptance through Expo Go

Automated Android/device E2E through Maestro is disabled by the owner decision and is not a
task or CI gate. The owner tests the application personally on a physical phone.

1. Connect the phone and computer to the same LAN and allow inbound connections to local API
   port `3100`.
2. Start the local API and Admin as documented in their READMEs.
3. Start Expo with the computer's LAN address, for example:

   ```text
   EXPO_PUBLIC_API_URL=http://192.168.1.105:3100 pnpm --dir apps/mobile start -- --lan
   ```

4. Open the QR code in Expo Go. Verify the real Backend categories, Products, prices, images,
   details and visibility, including loading/error/retry states. Admin mutations must be
   confirmed by reopening Mobile data; no mock catalog or fallback data is acceptable.

The historical Maestro flows and runners remain archived for traceability only and must not be
run unless the owner explicitly re-enables automated device acceptance.
