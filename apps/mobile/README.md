...
- `src/infrastructure/product-api-client.ts` is the public HTTP trust boundary for
  `GET /products`; `src/application/catalog/product.ts` and
  `src/presentation/catalog/product-shell.tsx` render the validated persisted Product name
  and RUB base price.
...
only name and formatted `basePriceMinor` and have no order or details action; `adminEnabled`
is not treated as proof of iiko availability or orderability. Product loading failures remain
